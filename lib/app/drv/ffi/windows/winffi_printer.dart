...
class WinFFIPrinter {
  ...
  int openPtrEvent(Pointer<Utf8> filePath) {
    // 外部関数（インターフェース）
    final int Function(Pointer<Utf8> filePath) interface = printDylib
  ...
  int outRec(Pointer<Utf16> textData, Pointer<Utf16> bcData) {
    // 外部関数（インターフェース）
    final int Function(Pointer<Utf16> textData, Pointer<Utf16> bcData) interface = printDylib
  ...
  void releasePtr() {
    // 外部関数（インターフェース）
    final void Function() interface = printDylib
