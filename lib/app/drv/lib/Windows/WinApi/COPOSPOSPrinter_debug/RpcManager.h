...
public:
    ...
    HRESULT OnInit();
    HRESULT OnDestroy();
    HRESULT OnSend(PTR_DATA_RCV* pData);
...
private:
    ...
    function<HRESULT(IRpc*)> InitCallback = [=](IRpc* callback)
    {
        ...
        hrServer = callback->InitServer(RpcIdlSnd_v1_0_s_ifspec, hThread, PRINTER_SND_PORT);
        hrClient = callback->InitClient(RpcIdlRcv_v1_0_c_ifspec, &m_hBinding, PRINTER_RCV_PORT);
