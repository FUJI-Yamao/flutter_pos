...
public:
    ...
    HRESULT OnInit();
    HRESULT OnDestroy();
    HRESULT OnSend(DRW_DATA_SND* pData);
...
private:
    ...
    function<HRESULT(IRpc*)> InitCallback = [=](IRpc* callback)
    {
        ...
        hrServer = callback->InitServer(RpcIdlRcv_v1_0_s_ifspec, hThread, DRAWER_RCV_PORT);
        hrClient = callback->InitClient(RpcIdlSnd_v1_0_c_ifspec, &m_hBinding, DRAWER_SND_PORT);
