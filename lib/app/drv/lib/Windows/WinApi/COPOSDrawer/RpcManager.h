...
public:
    ...
    HRESULT OnInit();
    HRESULT OnDestroy();
    HRESULT OnSend(DRW_DATA_RCV* pData);
...
private:
    ...
    function<HRESULT(IRpc*)> InitCallback = [=](IRpc* callback)
    {
        ...
        hrServer = callback->InitServer(RpcIdlSnd_v1_0_s_ifspec, hThread, DRAWER_SND_PORT);
        hrClient = callback->InitClient(RpcIdlRcv_v1_0_c_ifspec, &m_hBinding, DRAWER_RCV_PORT);
