...
STDMETHODIMP COPOSCashDrawer::Open( BSTR DeviceName, long *pRC )
{
    ...
    *pRC = SetRC( OPOS_E_NOEXIST );
    ...
    UINT nDeviceNameLen = wcslen(DeviceName);   // BSTRキャスト仕様
...
#else
    cw_strcpy( szBaseKey, "SOFTWARE\\WOW6432Node\\OLEforRetail\\ServiceOPOS\\" OPOS_CLASSKEY_CASH );    // TeraokaPOS仕様のレジストリルート
#endif
...
void COPOSCashDrawer::WriteTraceString( LPCTSTR pszPrefix, BSTR bstr )
{
    ...
    int nLen = wcslen(bstr);    // BSTRキャスト仕様
